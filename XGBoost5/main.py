...
class Xgboost3(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.use_kelly = True
        self.kelly_fraction = 0.5  # Half-Kelly for safety (fractional Kelly)
        self.max_position_size = 0.95  # Maximum position size
        self.min_position_size = 0.1   # Minimum position size
        
        # Track prediction history for Kelly calculation
        self.prediction_history = []
        self.actual_return_history = []
        self.max_history_length = 500  # Keep last 500 predictions for stats
    ...
    def OnData(self, data):
        ...
        if self.model is not None:
            try:
                ...
                kelly_size = self._calculate_kelly_position(prediction)
                ...
                if prediction > threshold and not self.Portfolio[self.symbol].Invested:
                    self.SetHoldings(self.symbol, kelly_size)
                    self.Debug(f"{self.Time} BUY - Predicted: {prediction:.6f}, Kelly Size: {kelly_size:.2%}")
                    ...
                    # Rebalance existing position based on new Kelly size
                    ...
                    if abs(current_holdings - kelly_size) > 0.1:
                        self.SetHoldings(self.symbol, kelly_size)
                        self.Debug(f"{self.Time} REBALANCE - New Kelly Size: {kelly_size:.2%}")
        ...
        if features is not None and data.ContainsKey(self.symbol):
            ...
            # Update prediction history if we made a prediction
            if self.model is not None and len(self.training_data) > 0:
                try:
                    # Get actual return from last bar to current bar
                    last_close = self.training_data[-1]['close']
                    current_close = bar.Close
                    actual_return = (current_close / last_close) - 1
                    
                    # Get last prediction if available
                    if hasattr(self, 'last_prediction'):
                        self._update_prediction_history(self.last_prediction, actual_return)
                except:
                    pass
    ...
    def _calculate_kelly_position(self, predicted_return):
        """
        ...
        # Need historical data to estimate variance
        if len(self.prediction_history) < 50:
            # Not enough data, use conservative position
            return self.min_position_size
        
        try:
            # Calculate win rate and average win/loss from historical predictions
            predictions = np.array(self.prediction_history[-500:])
            actual_returns = np.array(self.actual_return_history[-500:])
            ...
            wins = actual_returns[correct_direction]
            losses = actual_returns[~correct_direction]
            ...
            kelly_pct = np.clip(kelly_pct, self.min_position_size, self.max_position_size)
            
            # Additional safety: reduce size if prediction is uncertain
            # Scale by prediction magnitude
            prediction_confidence = min(abs(predicted_return) * 100, 1.0)  # Cap at 1.0
            kelly_pct = kelly_pct * (0.3 + 0.7 * prediction_confidence)  # At least 30% of Kelly
            
            return max(kelly_pct, self.min_position_size)
            
        except Exception as e:
            self.Debug(f"Kelly calculation error: {str(e)}")
            return self.min_position_size
    
    def _update_prediction_history(self, prediction, actual_return):
        """Update prediction history for Kelly calculation"""
        self.prediction_history.append(prediction)
        self.actual_return_history.append(actual_return)
        
        # Keep only recent history
        if len(self.prediction_history) > self.max_history_length:
            self.prediction_history = self.prediction_history[-self.max_history_length:]
            self.actual_return_history = self.actual_return_history[-self.max_history_length:]
    ...
    def OnEndOfAlgorithm(self):
        ...
        if len(self.prediction_history) > 0:
            ...
            actuals = np.array(self.actual_return_history)
            correct_direction = np.sign(predictions) == np.sign(actuals)
            win_rate = np.mean(correct_direction)
            self.Debug(f"Model Win Rate: {win_rate:.2%}")
            self.Debug(f"Total Predictions: {len(self.prediction_history)}")